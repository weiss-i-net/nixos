...
- `desktop.nix` — imports `niri`, `homeManager`, `core`, and `user` (so any host importing `desktop` gets base system config and the `jannik` account for free); also sets greetd+niri session, xkb layout, fonts, bluetooth, printing, upower, cursor env vars, desktop-environment packages (nautilus, gvfs, dconf), and the GNOME dark-theme preference applied user-agnostically via `home-manager.sharedModules`.
