{ self, inputs, ... }: {
  ...
  # Package-building function shared across hosts, so a host with a
  # different monitor layout (e.g. desktop) can build its own niri package
  # by calling this with its own `outputs` override, rather than a second
  # package hardcoded in this file.
  flake.lib.mkNiriPackage =
    {
      pkgs,
      lib,
      self',
    ...
    inputs.wrapper-modules.wrappers.niri.wrap {
      inherit pkgs;
      settings = mkNiriSettings { inherit outputs; };
    };
  ...
  perSystem =
    ...
    {
      packages.myNiri = self.lib.mkNiriPackage { inherit pkgs lib self'; };
    };
}
